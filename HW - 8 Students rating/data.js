const test = [
    ...
    },
    {
        ...
        marks: [
            89, 45, 39, 50, 83, 67, 88, 68, 31, 29, 64, 80, 55, 57, 77, 23, 47, 58, 21, 61, 37,
            75, 90, 64
        ]
    },
    {
    ...
    },
    {
        ...
        marks: [
            100, 88, 27, 28, 28, 15, 55, 60, 90, 60, 33, 30, 9, 89, 59, 73, 20, 28, 25, 15, 18, 24,
            52, 14
        ]
    },
    {
    ...
    {
        name: 'Javilour Michaile',
        specialty: 'Product Strategist',
        marks: [
            34, 87, 5, 54, 69, 52, 20, 50, 64, 86, 77, 69, 7, 13, 37, 61, 36, 77, 49, 24, 34, 64,
            36, 56
        ]
    }
];
...
function averageMarks (array) {
    for (let index = 0; index < array.length; index++) {
        let resultSum1 = 0;
        const element = array[index].marks;
        for (let j = 0; j < element.length; j++) {
            resultSum1 += Number(element[j]);
        }
        let averageMark = resultSum1 / array[index].marks.length;
        array[index].averageMark = averageMark;
    }
}

function studentForDeduction (array) {
    averageMarks(array);
    for (let index = 0; index < array.length; index++) {
        const element = array[index].averageMark;
        if (element < 50) {
            console.log(`Name: ${array[index].name};\nAverage Mark:  ${array[index].averageMark}`);
        }
    }
}
...
function mediana (array) {
    for (let index = 0; index < array.length; index++) {
        const element = array[index].marks.sort((a, b) => {
            return a - b;
        });
        if (element.length % 2 === 1) {
            let medianStudentScore = element[Math.round(element.length / 2)];
            array[index].medianStudentScore = medianStudentScore;
        } else  {
            let j = element.length / 2;
            let medianStudentScore = (element[j] + element[j + 1]) / 2;
            array[index].medianStudentScore = medianStudentScore;
...
function listOfTheBestStudents (array) {
    sort (array);
    array.forEach((e) => {
        console.log(`${e.name}: ${e.averageMark}`)
    })
}

// function topFiveStudents (array) {
//     sort (array);
//     array.slice([0], [5]).forEach((e) => {
//         console.log(`${e.name}: ${e.averageMark}`);
//     })
// }
...
function topFiveStudents (array) {
    sort (array);
    element = array.slice([0], [5]);
    for (let index = 0; index < element.length; index++) {
        console.log(`${(index + 1)}. ${element[index].name}: ${element[index].averageMark}`);
